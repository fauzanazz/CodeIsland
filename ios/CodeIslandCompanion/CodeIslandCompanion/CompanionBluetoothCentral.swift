...
extension CompanionBluetoothCentral: CBCentralManagerDelegate {
    nonisolated func centralManagerDidUpdateState(_ central: CBCentralManager) {
        Task { @MainActor in
            ...
            case .unsupported:
                self.scanning = false
                self.connectedPeripheralName = nil
                self.lastError = "This iPhone does not support Bluetooth"
            case .resetting:
                self.scanning = false
    ...
    nonisolated func centralManager(_ central: CBCentralManager, didFailToConnect peripheral: CBPeripheral, error: Error?) {
        Task { @MainActor in
            self.lastError = error?.localizedDescription ?? "Could not connect to the Mac Bluetooth summary channel"
            self.peripheral = nil
    ...
    nonisolated func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        Task { @MainActor in
            ...
            guard let service = peripheral.services?.first(where: { $0.uuid == Self.serviceUUID }) else {
                self.lastError = "CodeIsland Bluetooth service was not found"
                return
            }
    ...
    nonisolated func peripheral(
        ...
        Task { @MainActor in
            ...
            guard let characteristic = service.characteristics?.first(where: { $0.uuid == Self.notifyCharacteristicUUID }) else {
                self.lastError = "CodeIsland Bluetooth notification channel was not found"
                return
            }
